...
const ProofDetailPage = () => {
  ...
  const [proofDetail, setProofDetail] = useState<any>(null);
  ...
  useEffect(() => {
    const fetchProofData = async () => {
      try {
        ...
        const proofData = (response as any).result || response;
        setProofDetail(proofData);
      } catch (err) {
        console.error("인증 데이터 로딩 실패:", err);
  ...
  if (loading) {
    return (
      <div className="font-pretendard bg-[#f0f5ff] min-h-screen p-0 flex justify-center">
  ...
  const images =
    proofDetail && proofDetail.imageKeys && Array.isArray(proofDetail.imageKeys)
      ? proofDetail.imageKeys.map((key: string) => getImageUrl(key))
      : [];
  ...
  return (
    <div className="font-pretendard bg-[#f0f5ff] min-h-screen p-0 flex justify-center">
      <div className="container-mobile">
        ...
        <main className="flex-grow overflow-y-auto p-6 bg-[#f0f5ff] min-h-0">
          <ProofDetail
