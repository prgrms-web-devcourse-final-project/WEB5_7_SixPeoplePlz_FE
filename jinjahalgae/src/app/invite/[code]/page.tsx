...
import { Button, Modal, ModalActions, ModalSection, useAlert } from "@/components/ui";
        ...
        <ModalSection>
          <div className="space-y-3">
            <Button
              ...
              disabled={isLoading}
              fullWidth
              className="bg-[#fee500] hover:bg-[#fdd835] text-black font-semibold"
            >
...
export default function InvitePage(props: {
  ...
  const { executeWithRetry: checkInviteExistsWithRetry } = useRetryableApiCall(
    (code: string) => checkInviteExists(code),
    {
      maxRetries: 3,
      retryDelay: 100
    }
  ...
  const { executeWithRetry: verifyInvitePasswordWithRetry } = useRetryableApiCall(
    (params: { code: string; password: string }) => verifyInvitePassword(params.code, { password: params.password }),
    {
      maxRetries: 3,
      retryDelay: 100
    }
  ...
  const { executeWithRetry: getContractDetailByInviteWithRetry } = useRetryableApiCall(
    (params: { inviteCode: string; contractUuid: string }) => getContractDetailByInvite(params.inviteCode, params.contractUuid),
    {
      maxRetries: 3,
      retryDelay: 100
    }
  ...
  const { executeWithRetry: signContractAsSupervisorWithRetry } = useRetryableApiCall(
    (params: { contractId: number; signatureImageKey: string }) => signContractAsSupervisor(params.contractId, params.signatureImageKey),
    {
      maxRetries: 3,
      retryDelay: 100
    }
  ...
  const handlePasswordSubmit = useCallback(async () => {
    ...
    try {
      const verifyResult = await verifyInvitePasswordWithRetry({
        code: params.code,
        password,
      });
      ...
      if (verifyResult.success && verifyResult.result?.contractUuid) {
        // 계약서 상세 정보 가져오기
        const contractDetail = await getContractDetailByInviteWithRetry({
          inviteCode: params.code,
          contractUuid: verifyResult.result.contractUuid,
        });
        ...
        if (contractDetail.success && contractDetail.result) {
          ...
          setError("계약서 정보를 불러오는데 실패했습니다.");
        }
      } else {
        setError("비밀번호가 올바르지 않습니다.");
      }
    } catch (err: any) {
      // INVITE_NOT_FOUND 에러 코드인 경우 특별한 메시지 표시
      if (err?.code === "INVITE_NOT_FOUND") {
        setError("이미 시작되거나 만료된 계약입니다.");
      } else if (err?.message) {
  ...
  const checkAuthAndInvite = async () => {
    // 초대 링크 유효성 검사
    try {
      await checkInviteExistsWithRetry(params.code);
    } catch (err: any) {
      // INVITE_NOT_FOUND 에러 코드인 경우 특별한 메시지 표시
      if (err?.code === "INVITE_NOT_FOUND") {
        setError("이미 시작되거나 만료된 계약입니다.");
    ...
    if (isLoggedIn) {
      // 로그인된 상태라면 세션 스토리지에서 저장된 정보 확인
      const inviteData = sessionStorage.getItem("pendingInvite");
      if (inviteData) {
        try {
          const parsedData = JSON.parse(inviteData);
          if (parsedData.code === params.code && parsedData.step === "password") {
            // 저장된 비밀번호로 자동 인증 시도
            try {
              const verifyResult = await verifyInvitePasswordWithRetry({
                code: params.code,
  ...
  const handleSignComplete = async () => {
    ...
    try {
      ...
      await signContractAsSupervisorWithRetry({
