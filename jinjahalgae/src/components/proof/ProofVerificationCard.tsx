...
interface ProofVerificationCardProps {
  verification: {
    ...
    hasReProof?: boolean; // 재인증이 이미 존재하는지 여부
  };
  contractId: string | number;
  mode: "contractor" | "supervisor";
...
const ProofVerificationCard: React.FC<ProofVerificationCardProps> = ({
  ...
  const handleCardClick = () => {
    if (!verification.proofId) return;
    if (mode === "contractor") {
      router.push(`/contracts/${contractId}/proofs/${verification.proofId}`);
    } else {
      router.push(`/supervise/${contractId}/proofs/${verification.proofId}`);
    }
  };
