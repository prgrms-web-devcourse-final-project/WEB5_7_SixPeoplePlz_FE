...
import { createProof, createReProof } from "@/lib/api/proofs";
...
export function VerificationBottomSheet({
  ...
  const confirmSubmit = async () => {
    ...
    try {
      ...
      if (isReProof && proofId) {
        // 재인증: /api/proofs/{proofId}/again 엔드포인트 사용
        await createReProof(proofId, proofData);
      } else {
        // 일반 인증: /contracts/{contractId}/proofs 엔드포인트 사용
  ...
  const handleClose = () => {
    ...
    setShowConfirmModal(false);
    setIsLoading(false);
    // (추가) 재인증 모달일 경우 쿼리 파라미터를 제거하여 새로고침 시 모달이 다시 뜨지 않도록 함
    if (isReProof) {
      const searchParams = new URLSearchParams(window.location.search);
      searchParams.delete("action");
      searchParams.delete("proofId");
