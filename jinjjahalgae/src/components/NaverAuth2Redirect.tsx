/**
 * 네이버 OAuth 인가 코드 처리 컴포넌트
 * 인가 코드를 받아 백엔드 서버로 로그인 요청을 보냅니다.
 */

"use client";

import { useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { exchangeNaverCodeForToken } from "@/lib/actions/naver-auth";
import { useAlert } from "./ui";

const NaverAuth2Redirect = () => {
  const [code, setCode] = useState<string | null>(null);
  const [state, setState] = useState<string | null>(null);
  const router = useRouter();
  const { showAlert, AlertComponent } = useAlert();

  useEffect(() => {
    if (typeof window !== "undefined") {
      const urlParams = new URL(window.location.href).searchParams;
      const authCode = urlParams.get("code");
      const stateParam = urlParams.get("state");

      if (authCode && stateParam) {
        setCode(authCode);
        setState(stateParam);
      } else {
        // 인가 코드가 없는 경우 에러 처리
        console.error("네이버 인가 코드를 찾을 수 없습니다.");
        router.replace("/auth");
      }
    }
  }, [router]);

  const naverLogin = async () => {
    if (!code || !state) return;

    try {
      // 서버 액션을 통해 네이버 로그인 처리 (이미 백엔드 API 호출 포함)
      const result = await exchangeNaverCodeForToken(code, state);

      if (
        result.success &&
        result.result?.accessToken &&
        result.result?.refreshToken
      ) {
        // 토큰을 직접 저장 (중복 API 호출 방지)
        const { tokenManager } = await import("@/lib/auth");
        tokenManager.setAccessToken(result.result.accessToken);
        tokenManager.setRefreshToken(result.result.refreshToken);

        // API 클라이언트 재생성
        const { resetApiClient } = await import("@/lib/auth/auth-client");
        resetApiClient();

        console.log("✅ 네이버 로그인 완료 - 토큰 저장됨");

        // 웹뷰 환경에서 FCM 토큰 업데이트
        if (window.NativeAppInterface) {
          try {
            console.log("웹뷰 환경 감지 - FCM 토큰 요청 중...");
            const fcmToken = await new Promise<string | null>((resolve) => {
              window.handleFcmTokenResponse = (token: string | null) => {
                resolve(token);
              };
              window.NativeAppInterface!.requestFcmToken();
              
              // 3초 후 타임아웃
              setTimeout(() => resolve(null), 3000);
            });

            if (fcmToken) {
              console.log("FCM 토큰 받음:", fcmToken);
              // FCM 토큰을 백엔드에 업데이트
              const { authService } = await import("@/lib/auth");
              await authService.updateFcmToken(fcmToken);
            }
          } catch (error) {
            console.warn("FCM 토큰 업데이트 실패:", error);
          }
        }

        // 로그인 성공 후 리다이렉트 처리
        // state에서 리다이렉트 경로 추출 (STATE_STRING_{encodedPath} 형식)
        let redirectPath = null;
        if (state && state.startsWith("STATE_STRING_")) {
          const encodedPath = state.replace("STATE_STRING_", "");
          redirectPath = decodeURIComponent(encodedPath);
        }

        if (redirectPath) {
          router.replace(redirectPath);
        } else {
          router.replace("/");
        }
      } else {
        throw new Error(result.error || "로그인에 실패했습니다.");
      }
    } catch (error) {
      console.error("네이버 로그인 실패:", error);
      showAlert({
        message: "로그인에 실패했습니다. 다시 시도해주세요.",
        type: "error",
      });
      router.replace("/auth");
    }
  };

  useEffect(() => {
    if (code !== null && state !== null) {
      naverLogin();
    }
  }, [code, state]);

  return <AlertComponent />;
};

export default NaverAuth2Redirect;
